...
// noopActuator satisfies engine.Actuator; never invoked because the live Healer runs
// dry-run (Apply returns before touching the actuator). Live mode is strictly READ-ONLY.
type noopActuator struct{}

func (noopActuator) CascadeCircuitBreak(int64, int32) engine.HealResult { return engine.HealResult{} }
func (noopActuator) WholeAgentCleanup(int64, int32) engine.HealResult   { return engine.HealResult{} }
func (noopActuator) Requeue(int64) engine.HealResult                    { return engine.HealResult{} }
func (noopActuator) Sync(int64) engine.HealResult                       { return engine.HealResult{} }
func (noopActuator) Terminate(int64) engine.HealResult                  { return engine.HealResult{} }

// Poller watches ic-dev CloudWatch for failure-queue cascade bursts and drives the engine.
type Poller struct {
	client    *Client
	diagnoser engine.Diagnoser
	healer    *engine.Healer
	engine    string
...
func NewPoller(c *Client, diag engine.Diagnoser, engineName string) *Poller {
	h := engine.NewHealer(noopActuator{})
	h.DryRun = true
	return &Poller{client: c, diagnoser: diag, healer: h, engine: engineName,
...
// withHeartbeat runs fn (a blocking external call) while emitting a "heartbeat" SSE event
// every interval. Long CloudWatch/Loki queries can run silent for 30-45s+; with zero bytes
// on the wire, some client-side network paths (browser heuristics, local proxies/security
// software) intermittently treat the idle connection as dead, closing it before fn ever
// returns — the client then sees "Live stream disconnected" with no results. The dashboard
// doesn't listen for "heartbeat", so it's otherwise invisible; it exists purely to keep the
// stream demonstrably alive. Waits for the ticking goroutine to fully exit before returning,
// so it never emits concurrently with the caller's next emit() call.
func withHeartbeat(emit Emit, interval time.Duration, fn func()) {
	...
	go func() {
		...
		for {
			...
			case <-stop:
				return
			case <-t.C:
				emit("heartbeat", map[string]any{"ts": time.Now().Format("15:04:05.000")})
			}
		}
	}()
	fn()
...
func (p *Poller) pollDwell(ctx context.Context, emit Emit, spec dwellSpec) {
	...
	var err error
	withHeartbeat(emit, 10*time.Second, func() {
...
func (p *Poller) pollCascade(ctx context.Context, emit Emit) {
	...
	if p.victims != nil {
		...
		if verr != nil {
			hint := "check GRAFANA_URL/token"
			if os.IsTimeout(verr) || strings.Contains(verr.Error(), "deadline exceeded") {
				hint = "query timed out — the 24h scan over entitymanagement.* is large; try again or narrow the window"
			}
			emit("log", logLine("VICTIMS", fmt.Sprintf("Loki victim lookup failed (%s): %s", hint, verr.Error())))
		} else {
